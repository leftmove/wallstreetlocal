...
def analyze_filer(cik, filings, updated_stocks):
    stock_cache = {}
    for access_number in filings:
        ...
        for cusip in filing_stocks:
            ...
            if filing_stock.get("prices", 0).get("buy").get("time") >= buy_time:
                stock_cache[cusip] = filing_stock
            else:
                local_stock = stock_cache[cusip]
            # Needs some serious rethinking

            updated_stock = serialize_stock(local_stock, found_stock)
            log_stock = {"name": name, "message": "Created Stock", "identifier": cusip}

            yield stock_query, filing_stock, updated_stock, log_stock
