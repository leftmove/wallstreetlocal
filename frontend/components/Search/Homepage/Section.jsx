...
function Section({ className }) {
  const [state, dispatch] = useReducer(
    ...
    { search: "", results: [], focus: false }
  );

  //   const [cik, setCik] = useState(null);
  //   useEffect(() => {
  //     const hovered = state.results.find((r) => r?.hover === true) || null;
  //     if (hovered) {
  //       setCik(hovered.cik);
  //     } else {
  //       setCik(null);
  ...
  //   const { data } = useSWR(
  //     cik ? [server + "/filers/preview", cik] : null,
  ...
  //     }
  //   );
  const filer = data?.filer || examples.at(0);
  ...
  return (
    <section className={cn(className)}>
      <Search
        ...
        className="w-full font-semibold bg-white-one"
      />
      <div className="relative w-full mt-5">
        <Example
          {...filer}
          className="absolute top-0 z-30 animate-slide-up-1 a hover:top-1"
        />
        <Example
          {...examples.at(1)}
          className="absolute z-20 scale-95 animate-slide-up-2 hover:z-40 hover:top-12 top-10"
        />
        <Example
          {...examples.at(2)}
          className="absolute z-10 scale-90 animate-slide-up-3 hover:z-40 hover:top-28 top-24"
        />
      </div>
    </section>
  );
