    ...
    async get(key, fetchCache) {
        ...
        if (!data) {
            try {
                ...
                const item = items == null ? void 0 : items[key];
                if (!item || !item.value) {
                    throw new Error(`invalid item returned ${JSON.stringify({
                        item
                    })}`);
                }
                ...
                if (this.debug) {
                    console.log(`got fetch cache entry for ${key}, duration: ${Date.now() - start}ms, size: ${item.value.length}`);
                }
                if (data) {
