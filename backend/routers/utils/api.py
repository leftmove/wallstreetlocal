...
async def sec_stock_search(cik, access_number):
    ...
    res = requests.get(
        f"https://www.sec.gov/Archives/edgar/data/{cik}/{access_number_replace}/{access_number}-index.html", headers=headers)
    data = res.text
...
async def cusip_request(value):
    while True:
        ...
        res = requests.get(
            f"https://finnhub.io/api/v1/search", params=params)
        if res.status_code == 429:
            print("Rate Limited, Waiting 60 Seconds...")
            await asyncio.sleep(60)
        else:
