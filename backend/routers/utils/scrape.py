...
async def scrape_name(cusip, name, cik):
    ...
    if found_stock == None:
        try:
            data = await cusip_request(cusip)
            data = data["result"][0]
            name = data["name"]
            ticker = data["symbol"]

            stock = await scrape_stock(ticker, cusip)
            stock["update"] = True
...
async def scrape_keys(tickers, name):
    if tickers == []:
        try:
            data = await cusip_request(name)
        ...
        for ticker in tickers:
            try:
                stock_info = await ticker_request("OVERVIEW", ticker)
...
async def scrape_filer(data):
    filings, last_report, first_report = await scrape_filings(data)
    time = (datetime.now()).timestamp()

    name = data["name"]
    tickers = data["tickers"]
    name, info = await scrape_keys(tickers, name)
...
async def scrape_stock(ticker, cusip):
    try:
        stock_info = await ticker_request("OVERVIEW", ticker)
        stock_price = (await ticker_request("GLOBAL_QUOTE", ticker))["Global Quote"]
    except Exception as e:
        print(e)
